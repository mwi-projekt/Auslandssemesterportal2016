    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        if (result.data.length == 0) {
          popUpHtml =
            '<b id="popClose"><img src="images/Button Delete.png" id="smallImg"></b><br><p>Sie haben sich bereits für alle verfügbaren Auslandsuniversitäten für ihren Studiengang beworben.</p>';
        } else {
          var popUpHtml =
            '<div class="form-horizontal"><div class="form-group"><div class="col-md-12"><select class="inBox" id="selectUni">';
          for (var l = 0; l < result.data.length; l++) {
            // filtern von schon beworbenen unis
            if (splitUni.indexOf(result.data[l].uniTitel) != -1) continue;
            popUpHtml =
              popUpHtml + "<option>" + result.data[l].uniTitel + "</option>";
          }

          popUpHtml = popUpHtml + "</select></div></div>";

          popUpHtml +=
            '<div class="form-group"><div class="col-md-12"><select class="inBox" id="selectZeit">';
          //Zeitraum muss hier noch automatisch generiert werden
          popUpHtml =
            popUpHtml +
            "<option> Zeitraum auswählen </option><option> Sommersemester 2021 </option>";

          popUpHtml = popUpHtml + "</select></div></div>";

          popUpHtml +=
            '<div class="form-group"><div class="col-md-12"><select class="inBox" id="selectPrio">';
          ...
          popUpHtml +=
            '<div class="form-group"><div class="col-md-12"><button id="newBewProzessWahl" class="btn btn-success">Bestätigen</button></div></div></div>';
        }
    ...
    success: function (data) {
      ...
      for (let i = 0; i < result.length; i++) {
        ...
        htmlText =
          ...
          "<br/>" +
          "<br/>" +
          "Mögliche Studiengänge für diese Hochschule: " +
          result[i].studiengang +
...
function initBewerben() {
  $.ajax({
    ...
    success: function (result) {
      tabelle =
        '<table class="table table-bordered table-hover"><thead><tr><th>Universität</th><th>Status</th><th>Priorität</th><th colspan="2">Aktionen</th></tr></thead>';
      if (result.data.length == 0) {
        ...
        for (let i = 0; i < result.data.length; i++) {
          ...
          tabelle =
            ...
            instance_info.uni +
            "</td><td>" +
            instance_info.stepCounter +
            "</td>";
          tabelle += "<td>" + instance_info.prioritaet + "</td>";
          prioArr.push(instance_info.prioritaet);
          //Anlegen der Buttons
          if (
            instance_info.stepCounter === "Abgeschlossen" ||
            instance_info.stepCounter === "Auf Rückmeldung warten" ||
            instance_info.stepCounter === "Bewerbung wurde abgelehnt"
          ) {
            //Übersicht
            tabelle =
              tabelle +
              '<td align="center"><span class="btn fas fa-list" title="Übersicht" onclick="location.href=\'task_detail.html?instance_id=' +
            ...
            tabelle =
              ...
              "&uni=" +
              instance_info.uni +
              '&send_bew=true\'"></span><td align="center">';
            //Prozess löschen
            tabelle =
              tabelle +
              '<span uni="' +
              instance_info.uni +
              '" class="btn fas fa-trash btn-delete" title="Löschen" rid="' +
              (i + 1) +
              '"></span>';
          } else {
            //Fortsetzen
            tabelle =
              tabelle +
              '<td align="center"><span class="btn fas fa-arrow-right" title="fortsetzen" onclick="location.href=\'bewerben.html?instance_id=' +
              instance_info.instanceID +
              "&uni=" +
              instance_info.uni +
    ...
    error: function (result) {
      Swal.fire(
        "Fehler",
        "Beim Abrufen der laufenden Prozesse ist ein Fehler aufgetreten",
        "error"
      );
    },
  });
      ...
      if (result.value) {
        ...
        $.ajax({
          ...
          success: function (data) {
            ...
            Swal.fire(
              "Gelöscht!",
              "Der Prozess wurde erfolgreich gelöscht.",
              "success"
            );
          },
          error: function (error) {
            ...
            Swal.fire(
              "Fehler",
              "Der Prozess konnte nicht gelöscht werden",
              "error"
            );
          },
        });
