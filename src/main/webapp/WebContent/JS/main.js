...
// -------------------------------------------------------- CMS ------------------------------------------------------------------------------
		...
		if ($(this).text() === "Bearbeiten" || $(this).text() === "Abbrechen") {
			...
			case ('cmsPortal'):
				...
				$.ajax ({
					...
					data: {
						...
						$('.erfolgreich').html("Ihre �nderungen wurden erfolgreich in die Datenbank gespeichert. <br> Bitte laden sie die Seite erneut.");
						$('.erfolgreich').show();
			...
			case ('cmsAngebote'):
				if ($(this).parent().attr('id') === "angebotNeu") {
					if ($('#inStudiengangNeuAngebot').val() != "Studiengang w�hlen*" && isEmpty($('#inNeuAngebot').val()) != true && isEmpty($('#inNeuAngebotInfo').val()) != true && isEmpty($('#inMaps1').val()) != true && isEmpty($('#inMaps2').val()) != true && isEmpty($('#inMaps3').val()) != true) {
						var faqs = '';
	...
	$('#cmsBtnBerichtNeu').on('click', function() {
		...
		$('.berichte').append('<div class="row kurzbericht" id="kb' + (count+1) + '"><div class="col-md-6"><img src="images/noimg.jpg" id="imgWorld" /><label class="btn cmsBtn cmsBerichte btnBild" id="btnberichtBild' + (count+1) + '" style="float: right">Bild �ndern<input class="upload" type="file" id="berichtBild' + (count+1) + '" /></label></div><div class="col-md-6 cmsBerichte"><textarea class="inBox" style="width: 100%; height: 250px"></textarea></div>');
		$('.kurzbericht').show();
		$('.cmsBerichte').show();
	});
	...
	$('.btnAngebote').on('click', function(event) {
		if ($(this).attr('id') === "btnAddStudgang") {
			...
			$('#cmsAddAngebot').show();
			$('#cmsEditAngebot').hide();
			$('#inStudiengangNeuAngebot').html($('#selStudiengang').html().replace("Alle Angebote", "Studiengang w�hlen*"));
			for (var i = 1; i <= 1; i++) {
				for (var j = 1; j <= 4; j++) {
