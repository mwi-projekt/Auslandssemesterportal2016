					...
					for (var i = 0; i < row.length; i++){
						...
						if ((instance_info[2] === "Abgeschlossen")||(instance_info[2] === "Auf Rückmeldung warten")){
							...
							tabelle = tabelle + '<td><span class="glyphicon glyphicon-list" title="Übersicht" onclick="location.href=\'task_detail.html?instance_id=' + instance_info[0] + '\'"></span></td>';
								...
								//Übersicht
								tabelle = tabelle + '<td colspan="2"><span class="glyphicon glyphicon-list" title="Übersicht" onclick="location.href=\'task_detail.html?instance_id=' + instance_info[0] + '&send_bew=true\'"></span></td>';
								//Prozess löschen
								tabelle = tabelle + '<td><span uni="' + instance_info[1] + '" class="glyphicon glyphicon-trash btn-delete" title="Löschen" rid="' + (i+1) + '"></span></td>';
						} else {
							//Fortsetzen
							tabelle = tabelle + '<td colspan="2"><span class="glyphicon glyphicon-circle-arrow-right" title="fortsetzen" onclick="location.href=\'bewerben.html?instance_id=' + instance_info[0] + '&uni='+instance_info[1]+'\'"></span></td>';
							//Prozess löschen
							tabelle = tabelle + '<td><span uni="' + instance_info[1] + '" class="glyphicon glyphicon-trash btn-delete" title="Löschen" rid="' + (i+1) + '"></span></td>'
						}
						tabelle = tabelle + '</tr>'
