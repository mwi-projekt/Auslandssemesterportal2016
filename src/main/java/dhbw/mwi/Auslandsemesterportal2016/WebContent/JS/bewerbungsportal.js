var main = function() {
	...
	$('#newBewProzess').on('click', function() {
		...
		$.ajax({
			...
			data: {
				...
				if (isEmpty(splitUni) != true) {
					for (var i = 0; i < auslesen.length - 1; i++) {
						for (var k = 0; k < splitUni.length; k++) {
							auslesen[i] = auslesen[i].trim();
							// Diese Abfrage verhindert, dass Unis f�r die der Student sich beworben hat in der Auswahl angezeigt werden
				...
				popUpHtml = popUpHtml + '</select><label class="btn" id="newBewProzessWahl" style="margin-left: 10px;">Best�tigen</label>';
				if (popUpHtml.match('<option>') != '<option>') {
					popUpHtml = '<b id="popClose"><img src="images/Button Delete.png" id="smallImg"></b><br><p>Sie haben sich bereits f�r alle verf�gbaren Auslandsuniversit�ten f�r ihren Studiengang beworben.</p>';
				}
	...
	//Click-Listener f�r neuen Erfahrungbericht starten Button
	...
	$('.btn').on('mouseover', function() {
		var id = $(this).attr('id');
		if (id === "btnBewSave") {
			$('#textbox').text("Wenn du noch nicht alle Felder ausgef�llt hast, kannst du so deine Daten zwischenspeichern, um sp�ter weiterzumachen.");
		} else if (id === "btnBewWeiter") {
			$('#textbox').text("Alle Felder ausgef�llt und �berpr�ft? Super, dann geht's hier weiter.");
		}
		$('#textbox').show();
	});
	...
	//Wenn w�hrend des Bewerbungsprozess auf Zur�ck zur �bersicht geklickt wird
	...
	//Auswahlm�glichkeit im Bewerbungsportal zur Sortierung der Angebote
						...
						data: {
							...
							$.ajax({
								...
								data: {
									...
									auslesen = [];
									auslesen = result.split(';');
									$('#formPrStra�e').html(auslesen[0] + ' ' + auslesen[1]);
									...
									$.ajax({
										...
										data: {
											...
											auslesen = [];
											auslesen = result.split(';');
											$('#formThStra�e').html(auslesen[0] + ' ' + auslesen[1]);
											...
											$.ajax({
												...
												data: {
													...
													$('#formParStra�e').html(auslesen[3] + auslesen[4]);
