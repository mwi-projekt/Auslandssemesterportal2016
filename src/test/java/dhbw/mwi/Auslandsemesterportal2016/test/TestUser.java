    ...
    private static void checkUser(int role) {
        if (role == 3) {
            ...
            if (uID == -1) {
                ...
                SQL_queries.userRegister("UnitTest", "User", Util.HashSha256(Util.HashSha256(testUser_pw) + salt), salt,
                        3, testUser_mail, "UnitTest", "UnitTest", 0003, "000", "000", "UnitTest", "1");
            } else {
                testUser_id = uID;
            }
            ...
            if (uID == -1) {
                String salt = Util.generateSalt();
                SQL_queries.userRegister("UnitTest", "AuslandsMA", Util.HashSha256(Util.HashSha256(testAAMA_pw) + salt),
                        salt, 2, testAAMA_mail, "UnitTest", "UnitTest", 0002, "000", "000", "UnitTest", "1");
            } else {
                testAAMA_id = uID;
            }
            ...
            if (uID == -1) {
                String salt = Util.generateSalt();
                SQL_queries.userRegister("UnitTest", "Admin", Util.HashSha256(Util.HashSha256(testAdmin_pw) + salt),
                        salt, 1, testAdmin_mail, "UnitTest", "UnitTest", 0001, "000", "000", "UnitTest", "1");
            } else {
                testAdmin_id = uID;
            }
            if (!SQL_queries.userLogin(testAdmin_mail, SQL_queries.getSalt(testAdmin_mail), testAdmin_pw)[0]
                    .equals("1")) {
